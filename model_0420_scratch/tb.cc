	...
	for (int k = 0; k < NUM_TESTS; k ++) {
		...
		for (int row = print_row_start; row < print_row_end; row ++) {
			for (int col = print_col_start; col < print_col_end; col ++) {
				...
				//				layer_1_pointwise_0_output
				//				layer_2_pointwise_0_output
//				float x = clip(layer_0_0_output[ch][row][col]);
//				float y = round((x + 1.0)/d)*d - 1.0;
				cout << fixed << setprecision(3) << layer_0_0_output[ch][row][col] << "  ";
			}
			cout << endl;
		}
...
//		cout << "---------- SW Classifier above ------------" << endl << endl;
		...
		int16 out_buf_all_paa[1024][9*226] = {};
		//		float conv13_hw_output[1024][7][7];
		//////////////////////////////////////////
		////////////// THE FUNCTION //////////////
		FracNet(image_hw_0,
				conv3x3_all_from_host, conv1x1_all_from_host,
				weights_all_from_host,
				out_buf_all_paa,
...
//			for (int i = 0; i < 2034; i ++){
			for (int row = 0; row < 7; row ++){
				for (int col = 0; col < img_width; col ++){
					float outout_hls = out_buf_all_paa[ch][(row+2)*(img_width+1) + (col+1)];
					float outout_cbh = layer_0_0_output[ch][img_height - 7 + row][col];
					cout << fixed << setprecision(3) << outout_hls << " ";
					float err = abs(outout_hls - outout_cbh);
					total_err_2 += err;
					total_cnt += 1;
					if (err > 0.0000001) {
						err_cnt_2 += 1;
...
//				int outout = out_buf_all[ch][(row+2)*(img_width+1) + (col+1)];
//				int col = i%(img_width + 1) + 1;
//				int row = i/(img_width + 1) + 2;
				cout << endl;
			}
		}
		float mean_error_2 = total_err_2/total_cnt;
//		cout << endl << "Local number of errors C vs hw: " << local_err_cnt << "/" << print_dim*print_dim << endl;
		cout << "Total number of errors C vs hw: " << err_cnt_2 << "/" << total_cnt << endl;
		cout << fixed << setprecision(6) << "Mean error C vs hw: " << mean_error_2 << endl;
		cout << fixed << setprecision(6) << "Total absolute error C vs hw: " << total_err_2 << endl;
//		cout << fixed << setprecision(6) << "Maximum absolute pixel error C vs hw: " << max_err << endl<<endl;
			...
			for (int col = ceil(print_col_start/4.0); col < ceil(print_col_end/4.0); col ++) {
				for (int col_i = 0; col_i < 4; col_i ++) {
					if (col*4 + col_i >= print_col_start && col*4 + col_i < print_col_end) {
						uint128 ddr_tmp_0 = 0;
						ddr_tmp_0.range(127, 0) = DDR_ptr_pack[ch_ptr*img_height*img_width/4 + row*img_width/4 + col].range(col_i*128+127, col_i*128);
						uint4 tmp = 0;
		...
		for(int i = 0; i < img_ch; i ++){
			...
			for(int row = 0; row < img_height; row ++){
				for(int col = 0; col < ceil(img_width/4.0); col ++){
					for (int col_i = 0; col_i < 4; col_i ++) {
						if (col*4 + col_i >= print_col_start && col*4 + col_i < print_col_end) {
							uint128 ddr_tmp_0 = 0;
							ddr_tmp_0.range(127, 0) = DDR_ptr_pack[ch_ptr*img_height*img_width/4 + row*img_width/4 + col].range(col_i*128+127, col_i*128);
							...
							float x = clip(layer_0_0_output[i][row][col*4 + col_i]);
							float y = round((x + 1.0)/d)*d - 1.0;
