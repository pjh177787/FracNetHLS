...
void put_out_buf_all(
		int16 out_buf_all_paa[1024][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
		int ch_off)
{
	for (int ch = 0; ch < 32; ch ++){
		for (int i = 0; i < 2034; i ++){
			out_buf_all_paa[ch_off + ch][i] = out_buf_all[ch][i];
		}
	}
}
...
#pragma HLS INTERFACE m_axi depth=1048576 port=DDR_buf_pack offset=slave bundle=DDR512
