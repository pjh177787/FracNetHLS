...
{
#pragma HLS INLINE
	const FIX_WT scale = 7.5;
//	ap_ufixed<4, 4, AP_RND, AP_SAT> temp = (ap_ufixed<4, 4, AP_RND, AP_SAT>)((x+1)*scale);
...
#pragma HLS UNROLL
					FIX_1D_PACK tmp = 0;
					tmp.range(WEIGHT_BITS_OTHER-1, 0) = data_pack[i].range(WEIGHT_BITS_OTHER-1+j*32, j*32);
					weight_buffer[i*NUM_WT_PACKS_OTHER+j] = (FIX_WT)tmp;
	//		weight_buffer[j].range(WEIGHT_BITS_OTHER-1, 0) = data_pack.range(WEIGHT_BITS_OTHER-1+j*16, j*16);
		}
	}
}
...
#pragma HLS ARRAY_PARTITION variable=out_buf_sc complete dim=1
	...
	int col_lim = ceiling(H_fmap_out/4.0);
	Load_from_DDR:
...
#pragma HLS PIPELINE
			for (int col_i = 0; col_i < 4; col_i ++) {
				ddr_tmp[row][col*4 + col_i].range(127, 0) = ddr_stage[row*col_lim + col].range(col_i*128+127, col_i*128);
			}
		}
	}
...
//#pragma HLS LOOP_FLATTEN
//#pragma HLS PIPELINE
//			uint512 data = DDR_buf[ddr_channel_ptr*H_fmap_out*H_fmap_out + (row_tile_start + row)*H_fmap_out + col];
//			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
//				out_buf_sc[ch][row][col].range(3, 0) = data.range(ch*16+3, ch*16);
//
//			}
//		}
//	}
...
#pragma HLS PIPELINE II=2
			for (int col_i = 0; col_i < 4; col_i ++) {
				ddr_stage[row*col_lim + col].range(col_i*128+127, col_i*128) = ddr_tmp[row][col*4 + col_i].range(127, 0);
			}
		}
	}
//	DDR_buf[(row_tile_offset + row)*H_fmap_out + col].range(ch*16+3, ch*16)
...
#pragma HLS ARRAY_PARTITION variable=out_feature_t1 complete dim=1
	...
	const FIX_WT ddr_scale = 2.0/15.0;
...
#pragma HLS PIPELINE
			...
			// Shortcut
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				out_feature_t1[ch] += out_buf_sc[ch][row][col]*ddr_scale - 1;
			}
			...
			uint128 pack_tmp = 0;
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				uint4 tmp = to4bit(out_feature_t1[ch]);
				pack_tmp.range(ch*4+3, ch*4) = tmp.range(3, 0);
			}
			ddr_tmp[row][col] = pack_tmp;
...
#pragma HLS PIPELINE
					...
					for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++){
						FIX_FM_acc t = out_buf_sc[ch][row_start+ii][col_start+jj]*ddr_scale - 1;// [-1, 1]
//						FIX_FM_acc t = out_buf_sc[ch][row_start+ii][col_start+jj];
						tmp[ch] += t;
					}
				}
			}
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++){
				uint4 ttt = to4bit(tmp[ch]/(FIX_FM_acc)4.0);
//							uint4 tmp = to4bit((FIX_FM_acc)0.2);
				out_buf_sc[ch][row][col] = ttt;
			}
		}
	}
}
