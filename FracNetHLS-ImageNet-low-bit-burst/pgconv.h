...
void pg_conv3x3_tile( 
		uint32 msb_inputs[FEAT_MAP_DIM],
		uint32 weights[OUT_CHANNEL_PARALLELISM][3][3],
		FIX_BUF_ALL msb_outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS ARRAY_PARTITION variable=msb_window_buffer complete dim=0
	...
	FIX_BUF_ALL msb_partial_out_feature[OUT_CHANNEL_PARALLELISM];
...
#pragma HLS PIPELINE 
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				FIX_BUF_ALL msb_accumulation = 0;
				...
				for (int k_row=0; k_row<3; k_row++) {
					for (int k_col=0; k_col<3; k_col++) {
						...
						if(row_idx_pad>=1 && row_idx_pad<H_fmap_out+1 && col_idx_pad>=0 && col_idx_pad<H_fmap_out){
							...
							msb_accumulation += 2*compute_engine_32_1(msb_a, w) - 32;
						}
					}
				}
...
void pg_conv1x1_tile(
		uint32 inputs[FEAT_MAP_DIM],
		uint32 weights[OUT_CHANNEL_PARALLELISM],
		FIX_BUF_ALL outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS ARRAY_PARTITION variable=outputs complete dim=1
	...
	FIX_BUF_ALL partial_out_feature[OUT_CHANNEL_PARALLELISM];
...
//			}
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				...
				FIX_BUF_ALL accumulation = 2*compute_engine_32_1(act, w) - 32;
				if (c_in > 0){
					outputs[channel_pt][output_index] += accumulation;
				} else{
					outputs[channel_pt][output_index] = accumulation;
				}
