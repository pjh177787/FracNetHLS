...
static FIX_DDR out_buf_sc[OUT_CHANNEL_PARALLELISM][ROW_TILE_SIZE+1][BUF_WIDTH_0]; // shortcut 32*8*114 = 917504
static FIX_BUF_ALL out_buf_all[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)]; // all 32*10*226 = 2315240
...
{
	LOOP_GetImg_0:
	for (int i = 0; i < (ROW_TILE_SIZE+2)*224; i ++){
#pragma HLS PIPELINE
		int index_thermo = c_in*224*224 + row_tile_start*224 + i;
		int index_feature = row_tile_start*224 + i;
		feat_buf_all_1[index_feature] = image_thermo[index_thermo];
	}
	LOOP_GetImg_1:
	for (int row = 0; row < ROW_TILE_SIZE+2; row ++){
		for (int col = 0; col < 225; col ++){
#pragma HLS PIPELINE
			int index_thermo = (row_tile_start+row-1)*224 + col;
			int index_feature = (row_tile_start+row)*225 + col;
			if (row_tile_start+row >= 1 && row_tile_start+row <= 224 && col <= 223){
				feat_buf_all_0[index_feature] = feat_buf_all_1[index_thermo];
			} else {
...
#pragma HLS INTERFACE m_axi depth=151528 port=image_thermo offset=slave bundle=IMG
...
#pragma HLS INTERFACE m_axi depth=32768 port=DDR_buf_pack offset=slave bundle=DDR512
