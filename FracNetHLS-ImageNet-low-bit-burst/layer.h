...
#pragma HLS UNROLL
		...
		weight_buffer[j].range(WEIGHT_BITS_OTHER-1, 0) = data_pack.range(WEIGHT_BITS_OTHER-1+j*WEIGHT_BITS_OTHER, j*WEIGHT_BITS_OTHER);
	}
}
...
//	weights_all_ptr += pw_channels_after_tile;
//}
void write_1D_weights(
		uint512 layer_1D_weights[18],
		FIX_WT conv_0_threshold[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_weight[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_bias[OUT_CHANNEL_PARALLELISM],
		FIX_WT relu_shift_x[OUT_CHANNEL_PARALLELISM],
...
)
{
	...
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		conv_0_threshold[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[0].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		conv_1_weight[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[1].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		conv_1_bias[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[2].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		relu_shift_x[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[3].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		relu_shift_y[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[4].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		relu_weight[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[5].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		bn_weight[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[6].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
	for (int j = 0; j < NUM_WT_PACKS_OTHER; j ++) {
#pragma HLS UNROLL
		bn_bias[j].range(WEIGHT_BITS_OTHER-1, 0) = layer_1D_weights[7].range(WEIGHT_BITS_OTHER-1+j*16, j*WEIGHT_BITS_OTHER);
	}
}

void fetch_1D_weights(
		uint512 weights_all[2730],
		int weights_all_ptr_start,
		int c_out)
{
	...
	for (int i = 0; i < 8; i ++){
#pragma HLS PIPELINE
		ddr_stage[i] = weights_all[weights_all_ptr + i];
	}
}

void load_layer_1D_weights_conv( //burst needs to organize weights
		FIX_WT conv_0_threshold[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_weight[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_bias[OUT_CHANNEL_PARALLELISM],
		FIX_WT relu_shift_x[OUT_CHANNEL_PARALLELISM],
...
void load_layer_1D_weights_pw(//burst needs to organize weights
		FIX_WT conv_0_threshold[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_weight[OUT_CHANNEL_PARALLELISM],
		FIX_WT conv_1_bias[OUT_CHANNEL_PARALLELISM],
		FIX_WT relu_shift_x[OUT_CHANNEL_PARALLELISM],
...
#pragma HLS PIPELINE
				...
				for (int j = 0; j < NUM_WT_PACKS; j ++) {
					weight3x3_tile_buffer[i*NUM_WT_PACKS + j][row][col].range(WEIGHT_BITS-1, 0) = ddr_stage[index].range(WEIGHT_BITS-1+j*WEIGHT_BITS, j*WEIGHT_BITS);
				}
			}
		}
	}
...
void load_shortcut(
		FIX_DDR out_buf_sc[OUT_CHANNEL_PARALLELISM][ROW_TILE_SIZE + 1][BUF_WIDTH_0],
...
#pragma HLS ARRAY_PARTITION variable=out_buf_sc complete dim=1
	...
	int ddr_channel_ptr = out_channel_start/OUT_CHANNEL_PARALLELISM + row_tile_start;
	if (switch_bank) {
	...
	Load_from_ddr:
	for (int i = 0; i < H_fmap_out*(ROW_TILE_SIZE + 1)/4; i ++) {
#pragma HLS PIPELINE
		ddr_stage[i] = DDR_buf[ddr_channel_ptr+i];
	}

	Stage_for_shortcut:
	for (int row = 0; row < (ROW_TILE_SIZE + 1)/4; row ++) {
		for (int col = 0; col < H_fmap_out; col ++) {
#pragma HLS PIPELINE
			for (int row_i = 0; row_i < 4; row_i ++) {
				ddr_tmp[row][col].range(127, 0) = ddr_stage[row*H_fmap_out + col].range(row_i*128+127, row_i*128);
			}
		}
	}
...
#pragma HLS PIPELINE
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++){
				out_buf_sc[ch][row][col].range(3, 0) = ddr_tmp[row][col].range(ch*4+3, ch*4);
			}
		}
	}
}
...
void bn_relu_small(
		...
		uint512 DDR_buf[DDR_BUF_DIM],
		FIX_BUF_ALL out_buf_all[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS DEPENDENCE variable=ddr_tmp array inter false
#pragma HLS ARRAY_PARTITION variable=ddr_tmp cyclic factor=2 dim=1
...
#pragma HLS PIPELINE
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				out_feature_t0[ch] = out_buf_all[ch][(row*stride+odd+2)*(H_fmap_in+1)+col*stride+1];
			}

			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				out_feature_t0[ch] = bn_weight[ch]*out_feature_t0[ch] + bn_bias[ch];
			}

			uint128 pack_tmp = 0;
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				FIX_DDR tmp = out_feature_t0[ch];
			...
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				...
				uint2 result_t = to2bit(out_feature_t0[ch]);
	...
	for (int row = 0; row < (ROW_TILE_SIZE + (1 - odd)*(stride-1))/stride/4; row ++) {
		for (int col = 0; col < H_fmap_out; col ++) {
#pragma HLS PIPELINE
			for (int row_i = 0; row_i < 4; row_i ++) {
				ddr_stage[row*H_fmap_out + col].range(row_i*128+127, row_i*128) = ddr_tmp[row*4+row_i][col].range(127, 0);
			}
		}
	}
	for (int i = 0; i < H_fmap_out*(ROW_TILE_SIZE + (1 - odd)*(stride-1))/stride/4; i ++) {
#pragma HLS PIPELINE
		DDR_buf[i] = ddr_stage[i];
	}
}
