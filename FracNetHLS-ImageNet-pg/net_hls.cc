...
uint64 pg_buf_all[2][12996];//114*114
...
void load_buf_from_DDR(uint512* src, int src_offset, FIX_FM dest[BLK_DEPTH][9][9], int row_offset, int col_offset, int ch_offset)
{
...
void copy_buf_to_DDR(uint512* dest, int dest_offset, FIX_FM src[BLK_DEPTH][9][9], int row_offset, int col_offset, int ch_offset)
{
//#pragma HLS ARRAY_PARTITION variable=dest complete dim=1
    // ddr[2][16][16][32][7][7]
    // ddr[ch_off][row_off][col_off][c][h][w]
...
void load_weight_1x1_from_axi(uint64 dest[BLK_DEPTH], uint512 src[1000], int index)
{
    for (int cc = 0; cc < 8; cc ++) {
#pragma HLS pipeline
        uint512 DATA = 0;
        DATA.range(511, 0) = src[index + cc].range(511, 0);
        for (int c = 0; c < 8; c++) {
#pragma HLS unroll
            dest[cc*8+c] = DATA.range(63+c*64, c*64);
        }
    }
}
...
void load_weight_3x3_from_axi( uint64 dest[BLK_DEPTH][3][3], uint512 src[1000][3][3], int index)
{
// #pragma HLS ARRAY_PARTITION variable=dest complete dim=1

    // index: should be which the channel offset of each layer in groups of 4
// 	uint512 src_buf[8][3][3];
//     for (int cc = 0; cc < 8; cc ++) {
//         for (int m = 0; m < 3; m++) {
//             for (int n = 0; n < 3; n++) {
// #pragma HLS pipeline
        ...
        int weights_all_index)
{
    for (int coff = 0; coff < 4; coff ++) {
        load_weight_1x1_from_axi(weight_buf_1x1[0], conv_weight_1x1_all, weight_1x1_index);
    }
//    load_1x1_from_axi(bn_weight_buf[0], weights_all[weights_all_index]);
//    load_1x1_from_axi(bn_bias_buf[0], weights_all[weights_all_index + 1]);
//    load_1x1_from_axi(thres_buf[0], weights_all[weights_all_index + 2]);
...
//                }
            ...
            int b = ch_offset%2;
            pg_buf_all[1][index + col].range(b*32 + 31, b*32) = DATA1.range(31,0);
            pg_buf_all[0][index + col].range(b*32 + 31, b*32) = DATA0.range(31,0);
...
void load_input(int row, int col, int c, uint32 img[3*226*226])
{
	int i_start = c*226*226 + row*226*112 + col*112;
...
{
#pragma HLS ARRAY_PARTITION variable=FM_buf0 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=FM_buf1 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=FM_buf_acc0 dim=1 cyclic factor=32
#pragma HLS ARRAY_PARTITION variable=weight_buf_3x3 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=weight_buf_3x3 dim=2 cyclic factor=8
#pragma HLS ARRAY_PARTITION variable=weight_buf_1x1 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=weight_buf_1x1 dim=2 cyclic factor=8
...
#pragma HLS ALLOCATION instances=pgconv64_1x1              		limit=1 function
#pragma HLS ALLOCATION instances=pgconv64                  		limit=1 function
...
//#pragma HLS ARRAY_PARTITION variable=conv1_weights complete dim=2
    FIX_WT conv1_bn_weights[3][32];
    FIX_WT conv1_bn_bias[3][32];
...
#pragma HLS PIPELINE II=1
						...
						for (int cc = 0; cc < 8; cc ++) {
							...
							int ii = i;
							if (b > 2) ii += 16;
							weight_buf_3x3[c][ii][m][n] = DATA.range(32+i*32, i*32);
						}
					}
                }
            }
...
//	load_input(0, 0, 0, pg_buf[0], image_thermo);
	for (int row_off = 0; row_off < 2; row_off ++) {
		for (int col_off = 0; col_off < 2; col_off ++) {
			for (int ch_off = 0; ch_off < 3; ch_off ++){
				load_input(row_off, col_off, ch_off, image_thermo);
				for (int row = 0; row < 12; row ++) {
					for (int col = 0; col < 12; col ++) {
						pgconv64(pg_buf_all[1], pg_buf_all_in, weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112, stride);
			...
			#pragma HLS PIPELINE II=2
								for (int c = 0; c < 32; c ++) {
									FIX_FM_acc fm = FM_buf_acc0[c][row][col];
									FIX_FM_acc d0 = batch_norm(fm,  conv1_bn_weights[ch_off], conv1_bn_bias[ch_off]);
									uint2 d = (uint2) d0;
									pg_buf_all[1][buf_index + nn][c] = (uint2) d[1];
									pg_buf_all[0][buf_index + nn][c] = (uint2) d[0];
								}
							}
							buf_index += 114;
						}
...
//					load_buf_from_buf_all(row, col, 0, 0, 112);
					pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112, stride);
				}
...
//					load_buf_from_buf_all(row, col, 0, 0, 112);
					pgconv64_1x1(pg_buf_all[1], pg_buf_all[0], weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112);
				}
				int coo_cat = coo;
				if (coo > N_COI) {
					coo_cat = coo - N_COI;
    ...
    for (int cio = 0; cio < N_CIO; cio ++) {
		...
		for (int row = 0; row < N_SPI; row ++) {
			for (int col = 0; col < N_SPI; col ++) {
				...
				for (int cii = 0; cii < N_CII; cii ++) {
//					load_buf_from_buf_all(row, col, 0, 0, 112);
					pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 112, stride);
				}
...
//					load_buf_from_buf_all(row, col, 0, 0, 56);
					pgconv64_1x1(pg_buf_all[1], pg_buf_all[0], weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 56);
				}
				int coo_cat = coo;
				if (coo > N_COI) {
					coo_cat = coo - N_COI;
	 	...
	 	for (int row = 0; row < N_SPI; row ++) {
	 		for (int col = 0; col < N_SPI; col ++) {
				load_buf_from_DDR(DDR_buff_merge, 0, FM_buf0, row, col, cio);
	 			for (int cii = 0; cii < N_CII; cii ++) {
//					load_buf_from_buf_all(row, col, 0, 0, 56);
	 				pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 56, stride);
	 			}
...
//					load_buf_from_buf_all(row, col, 0, 0, 56);
 ...
 //					load_buf_from_buf_all(row, col, 0, 0, 28);
 					pgconv64_1x1(pg_buf_all[1], pg_buf_all[0], weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 28);
 	 			}
 	 			int coo_cat = coo;
 	 			if (coo > N_COI) {
 	 				coo_cat = coo - N_COI;
 ...
 //					load_buf_from_buf_all(row, col, 1, 0, 28);
 					pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 28, stride);
 	 			}
 ...
 //					load_buf_from_buf_all(row, col, 0, 0, 14);
 					pgconv64_1x1(pg_buf_all[1], pg_buf_all[0], weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 14);
 	 			}
 	 			int coo_cat = coo;
 	 			if (coo > N_COI) {
 	 				coo_cat = coo - N_COI;
 	 	...
 	 	for (int row = 0; row < N_SPI; row ++) {
 	 		for (int col = 0; col < N_SPI; col ++) {
 				load_buf_from_DDR(DDR_buff_merge, 0, FM_buf0, row, col, cio);
 	 			for (int cii = 0; cii < N_CII; cii ++) {
 //					load_buf_from_buf_all(row, col, 0, 0, 14);
 					pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 14, stride);
 	 			}
 ...
 //					load_buf_from_buf_all(row, col, 0, 0, 7);
 					pgconv64_1x1(pg_buf_all[1], pg_buf_all[0], weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 7);
 	 			}
 	 			int coo_cat = coo;
 	 			if (coo > N_COI) {
 	 				coo_cat = coo - N_COI;
 	 	...
 	 	for (int row = 0; row < N_SPI; row ++) {
 	 		for (int col = 0; col < N_SPI; col ++) {
 				load_buf_from_DDR(DDR_buff_merge, 0, FM_buf0, row, col, cio);
 	 			for (int cii = 0; cii < N_CII; cii ++) {
 //					load_buf_from_buf_all(row, col, 0, 0, 7);
 					pgconv64(pg_buf_all[1], pg_buf_all[0], weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, off_row, off_col, 7, stride);
 	 			}
 ...
 #pragma HLS ARRAY_PARTITION variable=linear_weight_buf dim=2 cyclic factor=32
