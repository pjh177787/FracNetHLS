...
)
{
    ...
    for (int cin=0; cin<in_channels; cin++) {
        ...
        #pragma HLS array_partition variable=msb_line_buffer dim=0 complete
        #pragma HLS array_partition variable=msb_window_buffer dim=0 complete
        #pragma HLS array_partition variable=lsb_line_buffer dim=0 complete
        #pragma HLS array_partition variable=lsb_window_buffer dim=0 complete
        for (int row=0; row<H_fmap_out+1; row++) {
            ...
            #pragma HLS PIPELINE
                ...
                #pragma HLS array_partition variable=msb_partial_out_feature dim=1 complete
                FIX_FM_acc lsb_partial_out_feature[OUT_CHANNEL_PARALLELISM];
                #pragma HLS array_partition variable=lsb_partial_out_feature dim=1 complete
