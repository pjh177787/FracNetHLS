...
#pragma HLS ARRAY_PARTITION variable=weight_tile_buffer complete dim=1
	...
	// After loading the image feat_buf_all should be full
...
//				int index_feature = c_in*226*225 + row*225 + col;
				int index_feature = row*3*225 + c_in*225 + col;
	...
	for (int row_t = 0; row_t < H_fmap_out/ROW_TILE_SIZE; row_t++){
		for (int c_out = 0; c_out < conv_out_channels/OUT_CHANNEL_PARALLELISM; c_out ++){
...
#pragma HLS ARRAY_PARTITION variable=l_0_0_weight complete dim=3
							weight_tile_buffer[ch][row][col] = l_0_0_weight[c_out][c_in][ch][row][col].range(31, 0);
						}
					}
				}
				...
				pg_conv3x3_tile(
						feat_buf_all, weight_tile_buffer, out_buf_t1,
						c_in, in_channels_after_pack, H_fmap_out, row_tile_start
				);
...
#pragma HLS PIPELINE
#ifdef SW_TEST
						float result = (float)bn_weight_buffer[c_out][ch]*out_buf_t1[ch][row*2+2][col*2+1] + (float)bn_bias_buffer[c_out][ch];
#else
						float result = (FIX_32_8)bn1_weight[c_out][ch]*out_buf_t1[ch][row*2+2][col*2+1] + (FIX_32_8)bn1_bias[c_out][ch];
#endif
