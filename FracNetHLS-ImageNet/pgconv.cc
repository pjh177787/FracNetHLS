...
void pgconv64_1bit(uint64 bottom1[12996],
//                uint64 bottom0[9][9],
				...
				for (int coo = 0; coo < CH_OUT_PAR; coo ++) {
					...
					for (int k_row = 0; k_row < 3; k_row ++) {
						for (int k_col = 0; k_col < 3; k_col ++) {
							uint64 a = bot1_WB[k_row][k_col];
							uint64 w = weights[w_i][k_row][k_col];
							FIX_FM_acc mul = compute_engine_64(a, w);
							sum += mul;
						}
					}
					top_buf[coo] += sum;
...
void pgconv64_1x1_1bit(uint64 bottom[12996],
...
#pragma HLS PIPELINE II=1
				...
				if (0 < row && row < 9) {
					...
					if (0 < col && col < 9) {
						...
						for (int coo = 0; coo < CH_OUT_PAR; coo ++) {
							uint8 sum = compute_engine_64(bot1_WB, weights[coo]);
							top_buf[coo] += sum;
						}
						for (int coo = 0; coo < CH_OUT_PAR; coo ++) {
							top[ch_start + coo][row][col] = top_buf[coo];
