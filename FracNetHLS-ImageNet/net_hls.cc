		...
		for (int col_off = 0; col_off < 2; col_off ++) {
			for (int ch_off = 0; ch_off < 3; ch_off ++){
				...
				for (int row = 0; row < 12; row ++) {
					for (int col = 0; col < 12; col ++) {
						pgconv64_1bit(pg_buf_all_in, weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112, stride);
			...
			#pragma HLS PIPELINE II=2
								for (int c = 0; c < 32; c ++) {
									...
									pg_buf_all[buf_index + nn][c] = (uint1)d0;
								}
							}
							buf_index += 114;
						}
...
//					load_buf_from_buf_all(row, col, 0, 0, 112);
					pgconv64_1bit(pg_buf_all, weight_buf_3x3[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112, stride);
				}
...
//					load_buf_from_buf_all(row, col, 0, 0, 112);
					pgconv64_1x1_1bit(pg_buf_all, weight_buf_1x1[0], thres_buf[0], FM_buf_acc0, row, col, 0, 0, 112);
				}
				int coo_cat = coo;
				if (coo > N_COI) {
					coo_cat = coo - N_COI;
