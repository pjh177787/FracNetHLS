...
void pg_conv3x3_tile( 
		...
		int16 msb_outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS ARRAY_PARTITION variable=msb_line_buffer complete dim=1
...
void pg_conv1x1_tile(
		...
		int16 outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
//#pragma HLS ARRAY_PARTITION variable=outputs complete dim=1
	...
	int16 partial_out_feature[OUT_CHANNEL_PARALLELISM] = {};
...
#pragma HLS PIPELINE
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				...
				int16 accumulation = 2*compute_engine_32_3(act, w) - 32;
