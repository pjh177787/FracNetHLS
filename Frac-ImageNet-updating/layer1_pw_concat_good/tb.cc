	...
	for (int k = 0; k < NUM_TESTS; k ++) {
		...
		for (int row = 0; row < 32; row ++) {
			for (int col = 0; col < 32; col ++) {
				cout << layer_1_pointwise_0_output[ch][row][col] << "  ";
			}
			cout << endl;
		}
		cout << "---------- SW above ------------" << endl;
		...
		for (int i = 0; i < 8314; i ++){
			for (int j = 0; j < 16; j ++){
				// LETs use 32-bits for weights first
				FIX_32_8 weight = weights_all[i*16 + j + 6];
				//				cout << weight << endl;
		...
		for(int i=0; i<64; i++){
			for(int j=0; j<112; j++){
				for(int k=0; k<112; k++){
					err = abs(DDR_ptr[i][j][k] - layer_1_pointwise_0_output[i][j][k]);
