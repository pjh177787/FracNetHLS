...
const uint4 lut16[16] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4};
const int4 lut16bip[16] = {-4, -2, -2, 0, -2, 0, 0, 2, -2, 0, 0, -2, 0, 2, 2, 4};
...
inline uint8 compute_engine_32_0(uint32 b, uint32 w)
{
	uint32 x = ~(b^w);
	x -= (x >> 1) & m1;
	x = (x & m2) + ((x >> 2) & m2);
...
inline uint8 compute_engine_32_1(uint32 b, uint32 w)
{
	uint32 x = ~(b^w);
	uint8 s = 0;
	for (int i = 0; i < 32; i ++){
...
inline uint8 compute_engine_32_2(uint32 b, uint32 w)
{
	uint32 x = ~(b^w);
	uint8 s = 0;
	x = x - ((x >> 1) & 0x55555555);
...
inline uint8 compute_engine_32_3(uint32 b, uint32 w)
{
	uint32 x = ~(b^w);
	uint8 s = 0;
	for (int i = 0; i < 8; i ++){
...
void pg_conv3x3_tile( 
		...
		FIX_FM_acc msb_outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS ARRAY_PARTITION variable=msb_outputs complete dim=1
	...
	uint32 msb_line_buffer[2][BUF_WIDTH_1] = {};
...
#pragma HLS ARRAY_PARTITION variable=msb_window_buffer complete dim=0
	...
	int msb_partial_out_feature[OUT_CHANNEL_PARALLELISM] = {};
...
#pragma HLS PIPELINE 
			...
			int read_index = c_in*(H_fmap_out+2)*(H_fmap_out+1) + (row+row_offset)*(H_fmap_out+1) + col;
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				if (c_in > 0){
					msb_partial_out_feature[channel_pt] = msb_outputs[channel_pt][row*(H_fmap_out+1) + col + out_buf_start];
				}
				else{
				...
				for (int k_row=0; k_row<3; k_row++) {
					for (int k_col=0; k_col<3; k_col++) {
						...
						if(row_idx_pad>=1 && row_idx_pad<H_fmap_out+1 && col_idx_pad>=0 && col_idx_pad<H_fmap_out){
							...
							msb_accumulation += 2*compute_engine_32_3(msb_a, w) - 32;
						}
					}
				}
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				msb_outputs[channel_pt][row*(H_fmap_out+1) + col + out_buf_start] = msb_partial_out_feature[channel_pt];
			}
...
void pg_conv1x1_tile(
		...
		FIX_FM_acc outputs[OUT_CHANNEL_PARALLELISM][(ROW_TILE_SIZE+2)*(BUF_WIDTH_1+1)],
...
#pragma HLS ARRAY_PARTITION variable=outputs complete dim=1
	...
	int partial_out_feature[OUT_CHANNEL_PARALLELISM] = {};
...
#pragma HLS PIPELINE
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				if (c_in > 0){
					partial_out_feature[channel_pt] = outputs[channel_pt][(row+2)*(H_fmap_out+1) + (col+1) + out_buf_start];
				}
				else{
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				...
				int accumulation = 2*compute_engine_32_3(act, w) - 32;
				partial_out_feature[channel_pt] += accumulation;
			}

			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				outputs[channel_pt][(row+2)*(H_fmap_out+1) + (col+1) + out_buf_start] = partial_out_feature[channel_pt];
			}
