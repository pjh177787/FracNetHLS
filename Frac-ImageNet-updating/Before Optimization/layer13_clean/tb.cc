...
int main()
{
	...
	for (int k = 0; k < NUM_TESTS; k ++) {
		...
		load_layer_output("layer_outputs/layer13_shiftbn2_output", arr_layer_output);
		cout << "--------- layer13_shiftbn2_output ---------" << endl;
		int ch = 0;
		for (int row = 0; row < img_height; row ++) {
			for (int col = 0; col < img_width; col ++) {
				cout << arr_layer_output[ch*img_height*img_width + row*img_width + col] << "  ";
			}
			cout << endl;
		}
		...
		//			cout << endl;
		//		}
		//		cout << "---------- SW L2 above ------------" << endl;
		for (int row = 0; row < img_height; row ++) {
			for (int col = 0; col < img_width; col ++) {
				cout << layer_13_pointwise_0_output[ch][row][col] << "  ";
			}
			cout << endl;
		}
		...
		for(int i=0; i<img_ch; i++){
			for(int j=0; j<img_height; j++){
				for(int k=0; k<img_width; k++){
					err = abs(layer_13_pointwise_0_output[i][j][k] - arr_layer_output[i*img_height*img_width + j*img_width + k]);
					//					err = abs(DDR_ptr[i][j][k] - layer_3_pointwise_0_output[i][j][k]);
					if (err > max_err) max_err = err;
					if (err > 0.1){
						err_cnt += 1;
						//						cout << "(" << i << ", " << j << ", " << k << ") " << DDR_ptr[i][j][k] << ", " << layer_3_pointwise_0_output[i][j][k] << endl;
						//						cout << "(" << i << ", " << j << ", " << k << ") " << DDR_ptr[i][j][k] << ", " << arr_layer_output[i*img_height*img_width + j*img_width + k] << endl;
					}
					total_err += err;
					total += 1;
				}
		...
		FracNet(image_hw, conv3x3_all_hw_new, conv1x1_all_hw_new, weights_all_hw, DDR_ptr, accelerator_output);
		...
		for(int i=0; i<img_ch; i++){
			for(int j=0; j<img_height; j++){
				for(int k=0; k<img_width; k++){
					err = abs(DDR_ptr[i][j][k] - arr_layer_output[i*img_height*img_width + j*img_width + k]);
					//					err = abs(DDR_ptr[i][j][k] - layer_3_pointwise_0_output[i][j][k]);
					if (err > max_err) max_err = err;
					if (err > 0.1){
						err_cnt += 1;
		...
		for(int i=0; i<img_ch; i++){
			for(int j=0; j<img_height; j++){
				for(int k=0; k<img_width; k++){
					//					err = abs(DDR_ptr[i][j][k] - arr_layer_output[i*img_height*img_width + j*img_width + k]);
					err = abs(DDR_ptr[i][j][k] - layer_13_pointwise_0_output[i][j][k]);
					if (err > max_err) max_err = err;
					if (err > 0.1){
						err_cnt += 1;
						//						cout << "(" << i << ", " << j << ", " << k << ") " << DDR_ptr[i][j][k] << ", " << layer_3_pointwise_0_output[i][j][k] << endl;}
		...
		cout << endl << "Total absolute error from C bench: " << total_err << endl;
		cout << "Total number of errors from C bench: " << err_cnt << "/" << total << endl;
		cout << "Maximum absolute pixel error from C bench: " << max_err << endl;
