	...
	for (int k = 0; k < NUM_TESTS; k ++) {
		...
		for (int row = 0; row < 32; row ++) {
			for (int col = 0; col < 32; col ++) {
				cout << layer_2_pointwise_0_output[ch][row][col] << "  ";
			}
			cout << endl;
		}
		cout << "---------- SW above ------------" << endl;
		...
		float layer_2_conv3x3_0_weight_flip[4096][3][3] = {};
		float layer_2_pointwise_0_weight_flip[8192] = {};
		//		uint32 layer_2_conv3x3_0_weight_pack32[64][2][3][3] = {};
		//		uint32 layer_2_conv3x3_0_weight_pack32_reorg[2][2][32][3][3] = {};
		//		uint512 layer_2_conv3x3_0_weight_pack512[2][2][2][3][3] = {};
		...
		//					for (int row = 0; row < 3; row ++){
		//						for (int col = 0; col < 3; col ++){
		//							float w = layer_2_conv3x3_0_weight[c_out][c_in*32+in_t][row][col];
		//							if (w > 0){
		//								layer_2_conv3x3_0_weight_pack32[c_out][c_in][row][col][in_t] = 1;
		//							} else {
		//								layer_2_conv3x3_0_weight_pack32[c_out][c_in][row][col][in_t] = 0;
		//							}
		//						}
		//					}
		//				}
		...
		//					for (int row = 0; row < 3; row ++){
		//						for (int col = 0; col < 3; col ++){
		//							uint32 w = layer_2_conv3x3_0_weight_pack32[c_out*32 + out_t][c_in][row][col];
		//							layer_2_conv3x3_0_weight_pack32_reorg[c_out][c_in][out_t][row][col] = w;
		//						}
		//					}
		//				}
		//			}
			...
			for (int out_t = 0; out_t < 32; out_t ++){
				for (int c_in = 0; c_in < 2; c_in ++){
					for (int in_t = 0; in_t < 32; in_t ++){
						for (int row = 0; row < 3; row ++){
							for (int col = 0; col < 3; col ++){
								int flip_index = c_out*32*2*32 + c_in*32*32 + out_t*32 + in_t;
								// [c_out][c_in][out_t][in_t] = uint32 [c_out][c_in][out_t]
								layer_2_conv3x3_0_weight_flip[flip_index][row][col] = layer_2_conv3x3_0_weight[c_out*32+out_t][c_in*32+in_t][row][col];
							}
						}
					}
				}
		...
		for (int i = 0; i < 8; i ++){
			for (int ch = 0; ch < 512; ch ++) {
				for (int row = 0; row < 3; row ++){
					for (int col = 0; col < 3; col ++){
						int index = i*512 + ch;
						float w = layer_2_conv3x3_0_weight_flip[index][row][col];
						if (w > 0) {
							layer_2_conv3x3_0_weight_hw[i][row][col][ch] = 1;
						} else {
							layer_2_conv3x3_0_weight_hw[i][row][col][ch] = 0;
						}
					}
				}
			}
			...
			for (int out_t = 0; out_t < 32; out_t ++){
				for (int c_in = 0; c_in < 2; c_in ++){
					for (int in_t = 0; in_t < 32; in_t ++){
						int flip_index = c_out*32*2*32 + c_in*32*32 + out_t*32 + in_t;
						// [c_out][c_in][out_t][in_t] = uint32 [c_out][c_in][out_t]
						layer_2_pointwise_0_weight_flip[flip_index] = layer_2_pointwise_0_weight[c_out*32+out_t][c_in*32+in_t];
		...
		for (int i = 0; i < 16; i ++){
			for (int ch = 0; ch < 512; ch ++) {
				int index = i*512 + ch;
				float w = layer_2_pointwise_0_weight_flip[index];
				if (w > 0) {
					layer_2_pointwise_0_weight_hw[i][ch] = 1;
				} else {
					layer_2_pointwise_0_weight_hw[i][ch] = 0;
				}
			}
		}

		FracNet(image_hw, conv3x3_all_hw, layer_2_conv3x3_0_weight_hw, layer_2_pointwise_0_weight_hw, conv1x1_all_hw, weights_all_hw, DDR_ptr, accelerator_output);
...
//		ch = ch + 1024;
		...
		for(int i=0; i<64; i++){
			for(int j=0; j<56; j++){
				for(int k=0; k<56; k++){
					err = abs(DDR_ptr[i][j][k] - layer_2_pointwise_0_output[i][j][k]);
					if (err > max_err) max_err = err;
					if (err > 0.1){
						err_cnt += 1;
						cout << "(" << i << ", " << j << ", " << k << ") " << DDR_ptr[i][j][k] << ", " << layer_2_pointwise_0_output[i][j][k] << endl;
					}
					total_err += err;
					total += 1;
				}
